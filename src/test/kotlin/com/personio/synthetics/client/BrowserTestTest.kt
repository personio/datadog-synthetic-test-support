...
internal class BrowserTestTest {
    ...
    fun `getCredentialsProvider returns AwsSecretsManagerCredentialsProvider if datadogCredentialsAwsArn is set`() {
        ...
        whenever(Config.testConfig.credentials)
            .thenReturn(
                Credentials(
                    ddApiKey = null,
                    ddAppKey = null,
                    awsRegion = "awsRegion",
                    datadogCredentialsAwsArn = "awsArn",
                ),
            )
    ...
    @Test
    fun `getCredentialsProvider returns ConfigCredentialsProvider if datadogCredentialsAwsArn is null and DD keys are not null`() {
        Config.testConfig = mock()
        whenever(Config.testConfig.credentials)
            .thenReturn(
                Credentials(
    ...
    private fun getGlobalVariableObject(
        id: String,
        name: String,
    ): SyntheticsGlobalVariable {
        val globalVariable =
            "{ \"id\": \"${id}\", \"description\": \"desc\", \"name\": \"${name}\", \"tags\": [], \"value\": {\"value\": \"abc\"}}"
